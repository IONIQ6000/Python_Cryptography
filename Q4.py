...
for j in range(length):  # This function encrypts the message in binary
    if message[j] == 1:
...
def mio(m, n):  # This function does the modular multiplicative inverse to find
    for z in range(1, n):
        if ((m % n) * (z % n)) % n == 1:
            return z
    return 1
...
for i in range(
        length):  # The private key generated by using the answer from the modular multiplicative inverse which is 8
    key = (pub[i] * y % n)
